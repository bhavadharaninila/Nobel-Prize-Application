...
app.post('/api/data/:category/:year/:id/like', (req, res) => {
  const { category,year, id } = req.params;
  const { firstname, surname, motivation, share } = req.body;
  const prize = data.prizes.find((p) => p.category === category && p.year === year);
  if (!prize) {
    return res.status(404).json({ message: "Category not found" });
  }
  const laureate = prize.laureates.find((l) => l.id === id);
  if (!laureate) {
    return res.status(404).json({ message: "Laureate not found" });
  } // Update laureate's details
   if(typeof laureate.like === 'undefined'){
    laureate.like = 1;
   }else {
    laureate.like += 1;
   }
